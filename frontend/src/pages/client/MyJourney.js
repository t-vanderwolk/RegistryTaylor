    ...
    <div className="space-y-8">
      ...
      <section className="rounded-[2.5rem] border border-blush/40 bg-white/95 p-8 shadow-soft backdrop-blur-sm">
        <header className="space-y-2">
          ...
          <p className="text-sm font-body text-charcoal/70">{completedMilestones} of {milestones.length} major moments complete · {milestonePercent}% of Taylor’s plan.</p>
        </header>
        <div className="mt-6 h-3 w-full rounded-full bg-blush/20">
          ...
          {focusItems.map((item) => (
            ...
            >
              <div className="space-y-3">
                <span className="inline-flex items-center rounded-full border border-blush/40 bg-white/70 px-3 py-1 text-[0.6rem] font-heading uppercase tracking-[0.3em] text-charcoal/80">
                  Concierge focus
                </span>
                <h3 className="font-heading text-lg text-charcoal">{item.title}</h3>
                <p className="text-sm font-body leading-relaxed text-charcoal/75">{item.description}</p>
              </div>
              <Link
                to={item.link}
