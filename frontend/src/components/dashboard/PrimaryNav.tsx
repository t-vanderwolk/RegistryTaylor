...
function toLinkHref(href: AppRoute): Route | UrlObject {
...
export default function PrimaryNav({
  ...
  ctaLink = DEFAULT_CTA,
  brandHref = "/dashboard/member" as AppRoute,
  showMobileNav = true,
}: PrimaryNavProps) {
    ...
    isMounted && mobileNavOpen
      ? createPortal(
          <div className="md:hidden">
            <div
              className="max-md:fixed max-md:inset-0 max-md:z-[60] max-md:bg-white max-md:overflow-y-auto"
    ...
    <>
      <nav className="sticky top-0 z-40 hidden border-b border-mauve-500/15 bg-ivory/80 px-6 py-3 shadow-md shadow-mauve-500/20 backdrop-blur-sm md:flex">
        <div className="mx-auto flex w-full max-w-screen-2xl items-center justify-between gap-6">
          <Link href={toLinkHref(brandHref)} className="flex items-baseline space-x-2" aria-label="Taylor-Made Baby Co. home">
          ...
          <div className="flex items-center gap-2 rounded-full border border-mauve-500/20 bg-white/75 px-3 py-1">
            {computedNavItems.map(({ label, href, Icon, isActive }) => {
              const active = isActive ? isActive(pathname) : pathname === href;
              return (
                <Link key={href} href={toLinkHref(href)} className={desktopLinkClasses(active)}>
      ...
      {showMobileNav ? (
        <>
          <nav className="md:hidden max-md:fixed max-md:top-0 max-md:left-0 max-md:right-0 max-md:z-40 flex items-center justify-between border-b border-mauve-500/10 bg-ivory/95 px-4 py-3 pr-20 shadow-sm shadow-mauve-500/10 backdrop-blur">
            <Link href={toLinkHref(brandHref)} className="flex items-center gap-2" aria-label="Taylor-Made Baby Co. home">
            ...
            <button
              ...
              className={[
                "inline-flex items-center gap-2 rounded-full border border-mauve-500/30 px-3 py-2 text-xs font-semibold uppercase tracking-[0.3em] transition md:hidden",
                "max-md:fixed max-md:top-4 max-md:right-4 max-md:z-[70]",
