...
const AcademyLayout = () => {
  ...
  const workbookPath = modules.length ? `/academy/workbook/${modules[0].id}` : "/academy/overview";

  return (
    <div className="min-h-screen bg-ivory/90 py-12">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 lg:px-0">
        <header className="rounded-[3rem] border border-mauve/20 bg-white/80 px-6 py-8 shadow-soft backdrop-blur-sm sm:px-10">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-heading uppercase tracking-[0.35em] text-mauve/70">
            ...
            <button
              ...
              className="hidden rounded-full border border-mauve/30 bg-ivory px-5 py-2 text-xs font-heading uppercase tracking-[0.28em] text-mauve shadow-soft transition hover:-translate-y-0.5 hover:bg-white/90 md:inline-flex"
            >
              Overview
            </button>
          </div>
          {(isMentor || isAdmin) && (
            <div className="mt-6 rounded-[2.5rem] border border-gold/30 bg-gold/10 px-4 py-4">
        ...
        <nav className="flex flex-col gap-4 rounded-[2.5rem] border border-ivory/80 bg-white/90 p-4 shadow-soft backdrop-blur-sm md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  "rounded-full px-4 py-2 text-xs font-heading uppercase tracking-[0.3em] transition",
                  isActive ? "bg-mauve/90 text-white shadow-soft" : "bg-ivory text-charcoal/70 hover:bg-ivory/80",
                ].join(" ")
              }
            >
              Overview
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  "rounded-full px-4 py-2 text-xs font-heading uppercase tracking-[0.3em] transition",
                  location.pathname.includes("/academy/workbook")
          ...
          <div className="flex flex-wrap gap-2 text-[0.6rem] font-heading uppercase tracking-[0.32em] text-charcoal/50">
        ...
        <div className="flex flex-col gap-6 rounded-[3rem] border border-charcoal/10 bg-white/80 p-6 shadow-soft backdrop-blur-sm">
          <div className="flex flex-wrap gap-3 overflow-x-auto">
            {modules.map((module) => (
              <NavLink
                key={module.id}
                to={`/academy/modules/${module.id}`}
                className={({ isActive }) =>
                  [
                    "flex items-center gap-3 rounded-full border px-4 py-2 text-sm font-body transition",
                    isActive
