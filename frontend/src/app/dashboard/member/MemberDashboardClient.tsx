...
function UpcomingEventCard({
  ...
  return (
    <section className="rounded-2xl border border-mauve-200/60 bg-gradient-to-br from-blush-100 via-ivory to-white p-4 shadow-mauve-card sm:p-6">
      ...
      <Link
        href={href}
        className="mt-4 inline-flex w-full items-center justify-center rounded-full bg-mauve-500/90 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white transition hover:bg-mauve-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mauve-300/60"
      >
        RSVP or view all
      </Link>
    </section>
...
function RegistryMiniCard({
  ...
  return (
    <section className="rounded-2xl border border-mauve-200/60 bg-white/90 p-4 shadow-mauve-card sm:p-6">
      ...
      <Link
        href={href}
        className="mt-4 inline-flex w-full items-center justify-center rounded-full border border-mauve-500/40 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-charcoal-600 transition hover:bg-blush-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mauve-300/60"
      >
        Open registry hub
      </Link>
    </section>
...
function ConciergeCard({
  ...
  return (
    <section className="rounded-2xl border border-mauve-200/60 bg-gradient-to-br from-ivory via-blush-100 to-white p-4 text-charcoal-700 shadow-mauve-card sm:p-6">
...
export default function MemberDashboardClient({ payload }: MemberDashboardClientProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-1 pt-2 sm:px-0 md:gap-8 lg:gap-10">
      <DashboardHeader {...header} />

      <div className="lg:grid lg:grid-cols-[minmax(0,1.6fr)_minmax(260px,0.95fr)] lg:items-start lg:gap-8 xl:gap-10">
        <div className="space-y-6 sm:space-y-7 md:space-y-8">
          <QuickAccessCards {...quickAccess} />
          <ProgressOverview {...progress} ctaHref={quickAccess.academy.href} />
          <LearnSpotlight modules={learn.modules} />
          <AnnouncementsFeed announcements={announcements} />
          <MessagesPanel {...messages} />
        </div>
        <aside className="mt-8 space-y-6 sm:space-y-7 lg:mt-0 lg:space-y-7 xl:space-y-8 lg:pl-6 lg:pt-2 lg:sticky lg:top-6">
          <ReflectionPanel reflections={reflections} communityHighlights={communityHighlights} />
          <UpcomingEventCard events={events} href={MEMBER_DASHBOARD_ROUTES.events as Route} />
          <RegistryMiniCard registry={registry} href={quickAccess.registry.href} />
          <ConciergeCard mentorName={payload.profile.mentorName} href={quickAccess.mentor.href} />
        </aside>
      </div>
    </div>
  );
